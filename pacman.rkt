...
(define (last-scene gs)
  (cond [(equal? (ghost->pos (gs-gt gs)) (sprite->pos (gs-pm gs)))
...
(check-expect (local [(define map (list (make-pos 0 0)
                                        (make-pos 1 0)
                                        (make-pos 3 0)
                                        (make-pos 4 0)))]
                (A-star (make-pos 0 0) (make-pos 3 0) map))
              false)
(check-expect (local [(define map (list (make-pos 0 0)
                                        (make-pos 1 0)
                                        (make-pos 2 0)
...
(check-expect (local [(define map (list (make-pos 0 0)
                ...
                (A-star (make-pos 0 0) (make-pos 1 3) map))
              (list (make-pos 0 0)
                    (make-pos 0 1)
                    (make-pos 1 1)
                    (make-pos 1 2)
                    (make-pos 1 3)))
(check-expect (local [(define map (list (make-pos 0 0) (make-pos 1 0)
                                        (make-pos 0 1) (make-pos 1 1)
                                        (make-pos 1 3) 
                                        (make-pos 1 4)))]
                (A-star (make-pos 0 0) (make-pos 1 4) map))
              false)
(check-expect (local [(define map (list (make-pos 0 0) (make-pos 1 0) (make-pos 2 0)
                                        (make-pos 0 1) (make-pos 1 1) (make-pos 2 1)
                                        (make-pos 0 2)                (make-pos 2 2)
                                        (make-pos 0 3) (make-pos 1 3) (make-pos 2 3)
                                        ))]
                (A-star (make-pos 0 2) (make-pos 2 1) map))
              (list (make-pos 0 2) (make-pos 0 1) (make-pos 1 1) (make-pos 2 1)))
(check-expect (local [(define map (list (make-pos 0 0) (make-pos 1 0) (make-pos 2 0)
                                        (make-pos 0 1) (make-pos 1 1) (make-pos 2 1)
                                        (make-pos 0 2)                (make-pos 2 2)
                                        (make-pos 0 3) (make-pos 1 3) (make-pos 2 3)
                ...
                (A-star (make-pos 0 2) (make-pos 2 1) map))
              (list (make-pos 0 2) (make-pos 0 1) (make-pos 1 1) (make-pos 2 1)))
(check-expect (local [(define map (list (make-pos 0 0) (make-pos 1 0) (make-pos 2 0) (make-pos 3 0)
                                        (make-pos 3 1)
                                        (make-pos 0 2) (make-pos 1 2) (make-pos 2 2) (make-pos 3 2)))]
                (A-star (make-pos 1 0) (make-pos 1 2) map))
              (list (make-pos 1 0) (make-pos 2 0) (make-pos 3 0) (make-pos 3 1) (make-pos 3 2) (make-pos 2 2) (make-pos 1 2)))
(check-expect (local [(define map 
                        (list
                         (make-pos 0 0) (make-pos 1 0)                (make-pos 3 0) (make-pos 4 0)
                         (make-pos 0 1) (make-pos 1 1)                (make-pos 3 1) (make-pos 4 1)
                         (make-pos 0 2)                                              (make-pos 4 2)
                ...
                (A-star (make-pos 0 2) (make-pos 3 1) map))
              (list (make-pos 0 2)  (make-pos 0 3) (make-pos 1 3) (make-pos 2 3)
                    (make-pos 3 3) (make-pos 4 3) (make-pos 4 2) (make-pos  4 1 ) (make-pos 3 1)))
  ...
  (local [;; WorklistEntry is (make-wle Pos Pos Number Number Number)
          ...
          (define (neighbour p bd)
            (local [(define x (pos-x p))
                    (define y (pos-y p))
                    (define (in? n) (member n bd))]
              (filter in? (list (make-pos (add1 x) y)
                                (make-pos (sub1 x) y)
                                (make-pos x (add1 y))
                                (make-pos x (sub1 y))))))
...
(define (tick-ghost ghost pm bd t)
  (if (zero? (modulo t 6))
      ghost
      (if (integer? t)
          (if (equal? (ghost->pos ghost) (sprite->pos pm))
              ghost
              (local [(define p 
...
(define (game-over? gs)
  (or (zero? (count-dots (gs-board gs)))
      (equal? (ghost->pos (gs-gt gs)) (sprite->pos (gs-pm gs)))))
...
(define (render-ghost ghost img t)
  (local [(define GT-IMG
            (cond [(= 1 (modulo t 4)) (circle 10 "solid" "red")]
                  [(= 2 (modulo t 4)) (circle 10 "solid" "orange")]
